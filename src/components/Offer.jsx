...
function Offer() {
  ...
  useEffect(() => {
    if(isSelected === 1) {
      setTitle(lang ? "Car transport for individuals" :"Transport dla klientów indywidualnych")
      setText(
        lang ?
    ...
    <div className={menu ? "offer offer__active" : "offer"}>
      ...
      <div className="offer__info">
        <div className="offer__options">
          ...
          className={isSelected === 1 ? "activeBtn offer__btn" : "offer__btn "} onClick={()=> setIsSelected(1)}>{lang ? "INDIVIDUAL": "INDYWIDUALNE"}</motion.button>
          <motion.button initial={{x:1000}} 
             animate={{x: 0}} 
             exit={{x:1000}}
             transition={{duration: 1}}
          className={isSelected === 2 ? "activeBtn offer__btn" : "offer__btn "} onClick={()=> setIsSelected(2)}>{lang ? "SET": "ZESTAWY"}</motion.button>
          <motion.button 
              initial={{x:1000}} 
             animate={{x: 0}} 
             exit={{x:1000}}
             transition={{duration: 1}}
          className={isSelected === 3 ? "activeBtn offer__btn" : "offer__btn "} onClick={()=> setIsSelected(3)}>{lang ? "TUG": "HOLOWANIE"}</motion.button>
        </div>
